<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Team</title>
    <script src="https://kit.fontawesome.com/3606fe9745.js" crossorigin="anonymous"></script>
</head>
<link rel="stylesheet" href="../WebStyle.css">
<body>          
    <?php include '../view/header.php'?>
    <div class="players-bNE">
        <div class="player-Carlo">
            <div class="playerNickname-Carlo">
                    <a href="#" aria-current="page" class="team-player-Carlo"><img src="../images/BabaCarloo.png" alt="BabaCarloo" style="width: 240px; height:210px;"></a>
                </div>
                <div class="text-per-player">
                    <h2>Dionis 'sinnopsyy' Budeci <img src="../images/XK.webp" alt=""></h2>
                    <p>Dionisi një lojtar profesional ne fushen e lojes CSGO.</p>
                    <h4>Ai është pjesë e ekipës Bad News Eagles</h4>
                    <h4>Së fundmi ata me ekipën janë kualifikuar në Rio Major ku kanë triumfuar në Legends Stage</h4>
                    <br>
                    <h3>Statistikat</h3>
                    <h4>Rating 2.0 &nbsp;:&nbsp; 1.02</h4>
                    <h4>Kills per round &nbsp;:&nbsp; 0.68</h4>
                    <h4>Headshots &nbsp;:&nbsp; 53.8%</h4>
                    <h4>Maps played &nbsp;:&nbsp; 51</h4>
                    <h4>Deaths per round &nbsp;:&nbsp; 0.70</h4>
                    <h4>Rounds contributed &nbsp;:&nbsp; 65.6%</h4>
                    <br>
                    <a href="https://www.instagram.com/dionisbudeeci"><i class="fa-brands fa-instagram"></i></a>
                    <a href="https://www.twitter.com/sinnopsyy_"><i class="fa-brands fa-twitter"></i></a>
                    <a href="https://www.twitch.tv/sinnopsyy"><i class="fa-brands fa-twitch"></i></a>
                </div>
        </div>
        <div class="player-BabaSchule">
                <div class="playerNickname-BabaSchule">
                    <a href="#" aria-current="page" class="team-player-BabaSchule"><img src="../images/BabaSchule.png" alt="BabaSchule" style="width: 240px; height:210px;"></a> 
                </div>
                <div class="text-per-player">
                    <h2>Rigon 'rigoN' Gashi <img src="../images/CH.webp" alt=""> <img src="XK.webp" alt=""></h2>
                    <p>Rigoni një super lojtar ne fushen e lojes CSGO.</p>
                    <h4>Ai është pjesë e ekipës Bad News Eagles</h4>
                    <h4>Së fundmi ata me ekipën janë kualifikuar në Rio Major ku kanë triumfuar në Legends Stage</h4>
                    <br>
                    <h3>Statistikat</h3>
                    <h4>Rating 2.0 &nbsp;:&nbsp; 0.99</h4>
                    <h4>Kills per round &nbsp;:&nbsp; 0.68</h4>
                    <h4>Headshots &nbsp;:&nbsp; 48.5%</h4>
                    <h4>Maps played &nbsp;:&nbsp; 51</h4>
                    <h4>Deaths per round &nbsp;:&nbsp; 0.75</h4>
                    <h4>Rounds contributed &nbsp;:&nbsp; 66.9%</h4>
                    <br>
                    <a href="https://www.twitter.com/rigoNcs"><i class="fa-brands fa-twitter"></i></a>
                    <a href="https://www.twitch.tv/rigoncs"><i class="fa-brands fa-twitch"></i></a>
                </div> 
        </div>
        <div class="player-Huanita">
                <div class="playerNickname-Huanita">
                    <a href="#" aria-current="page" class="team-player-Huanita"><img src="../images/Huanita.png" alt="Huanita" style="width: 240px; height:210px;"></a>
                </div>
                <div class="text-per-player">
                    <h2>Flatron 'juanflatroo' Halimi <img src="../images/XK.webp" alt=""></h2>
                    <p>Flatro një super lojtar ne fushen e lojes CSGO.</p>
                    <h4>Ai është pjesë e ekipës Bad News Eagles</h4>
                    <h4>Së fundmi ata me ekipën janë kualifikuar në Rio Major ku kanë triumfuar në Legends Stage</h4>
                    <br>
                    <h3>Statistikat</h3>
                    <h4>Rating 2.0 &nbsp;:&nbsp; 1.05</h4>
                    <h4>Kills per round &nbsp;:&nbsp; 0.73</h4>
                    <h4>Headshots &nbsp;:&nbsp; 53.3%</h4>
                    <h4>Maps played &nbsp;:&nbsp; 51</h4>
                    <h4>Deaths per round &nbsp;:&nbsp; 0.72</h4>
                    <h4>Rounds contributed &nbsp;:&nbsp; 67.2%</h4>
                    <br>
                    <a href="https://www.instagram.com/flatronhalimi"><i class="fa-brands fa-instagram"></i></a>
                    <a href="https://www.twitter.com/juanflatrooo"><i class="fa-brands fa-twitter"></i></a>
                    <a href="https://www.twitch.tv/juanflatroo"><i class="fa-brands fa-twitch"></i></a>
                </div>  
        </div>
        <div class="player-XHiiieks">
                <div class="playerNickname-XHiiieks">
                    <a href="#" aria-current="page" class="team-player-XHiiieks"><img src="../images/XHiiieks.png" alt="XHiiieks" style="width: 240px; height:210px;"></a>
                </div>
                <div class="text-per-player">
                    <h2>Genc 'gxx-' Kolgeci <img src="../images/XK.webp" alt=""></h2>
                    <p>Genci një super lojtar ne fushen e lojes CSGO.</p>
                    <h4>Ai është pjesë e ekipës Bad News Eagles</h4>
                    <h4>Së fundmi ata me ekipën janë kualifikuar në Rio Major ku kanë triumfuar në Legends Stage</h4>
                    <br>
                    <h3>Statistikat</h3>
                    <h4>Rating 2.0 &nbsp;:&nbsp; 1.06</h4>
                    <h4>Kills per round &nbsp;:&nbsp; 0.68</h4>
                    <h4>Headshots &nbsp;:&nbsp; 29.8%</h4>
                    <h4>Maps played &nbsp;:&nbsp; 51</h4>
                    <h4>Deaths per round &nbsp;:&nbsp; 0.60</h4>
                    <h4>Rounds contributed &nbsp;:&nbsp; 71.0%</h4>
                    <br>
                    <a href="https://www.instagram.com/genckolgeci"><i class="fa-brands fa-instagram"></i></a>
                    <a href="https://www.twitter.com/gxxcsgo"><i class="fa-brands fa-twitter"></i></a>
                    <a href="https://www.twitch.tv/gxxcsgo"><i class="fa-brands fa-twitch"></i></a>
                </div>
        </div>
        <div class="player-BabaQaz">
                <div class="playerNickname-BabaQaz">
                    <a href="#" aria-current="page" class="team-player-BabaQaz"><img src="../images/BabaQaz.png" alt="BabaQaz" style="width: 240px; height:210px;"></a>
                </div>
                <div class="text-per-player">
                    <h2>Sener 'SENER1' Mahmuti <img src="../images/XK.webp" alt=""></h2>
                    <p>Seneri një super lojtar ne fushen e lojes CSGO.</p>
                    <h4>Ai është pjesë e ekipës Bad News Eagles</h4>
                    <h4>Së fundmi ata me ekipën janë kualifikuar në Rio Major ku kanë triumfuar në Legends Stage</h4>
                    <br>
                    <h3>Statistikat</h3>
                    <h4>Rating 2.0 &nbsp;:&nbsp; 0.96</h4>
                    <h4>Kills per round &nbsp;:&nbsp; 0.61</h4>
                    <h4>Headshots &nbsp;:&nbsp; 50.2%</h4>
                    <h4>Maps played &nbsp;:&nbsp; 51</h4>
                    <h4>Deaths per round &nbsp;:&nbsp; 0.66</h4>
                    <h4>Rounds contributed &nbsp;:&nbsp; 67.8%</h4>
                    <br>
                    <a href="https://www.instagram.com/senermahmutii"><i class="fa-brands fa-instagram"></i></a>
                    <a href="https://www.twitter.com/SENER1CSGO"><i class="fa-brands fa-twitter"></i></a>
                    <a href="https://www.twitch.tv/sener1"><i class="fa-brands fa-twitch"></i></a>
                </div>
        </div>
    </div>
    <?php include '../view/footer.php'?>
</body>
</html>